...
const ListCombo = () => {
    ...
    const handleDecrease = (id) => {
        setQuantities((prev) => ({
            ...prev,
            [id]: Math.max((prev[id] || 0) - 1, 0), // Đảm bảo không giảm xuống dưới 0
        }));
    };
    
    const handleQuantityChange = (id, value) => {
        const newValue = Math.max(Number(value), 0); // Đảm bảo giá trị tối thiểu là 1
                                    ...
                                    return (
                                        <div key={food.id} className="flex items-stretch mb-4 w-1/3 group pb-10"> {/* Thêm class group tại đây */}
                                            ...
                                            <div>
                                                ...
                                                <div className="flex mt-12 pl-[15px] group hover:bg-yellow-500"> {/* Thêm class group và hover:bg-yellow-500 */}
                                                    <button 
                                                        className="bg-gray-300 text-black w-[30px] p-1 rounded-l group-hover:bg-yellow-500"  {/* Áp dụng hover cho nút */}
                                                    ...
                                                    <input 
                                                        ...
                                                        className="text-center w-[80px] border border-gray-300 bg-gray-300 group-hover:bg-yellow-500"  {/* Áp dụng hover cho input */}
                                                        style={{ appearance: 'none', MozAppearance: 'textfield', WebkitAppearance: 'none' }} // Loại bỏ các mũi tên
                                                    />
                                                    <button 
                                                        className="bg-gray-300 text-black w-[30px] p-1 rounded-r group-hover:bg-yellow-500"  {/* Áp dụng hover cho nút */}

...
const ListCombo = ({ onSelectCombos, setSelectedCinema }) => {
    ...
    const handleIncrease = (id) => {
        ...
        updateSelectedCombos(id, quantities[id] + 1);
    };
    ...
    const handleQuantityChange = (id, value) => {
        ...
        // Cập nhật selectedCombos
        updateSelectedCombos(id, newValue);
    };

    const updateSelectedCombos = (id, quantity) => {
        const updatedCombos = Object.entries(quantities).map(([comboId, qty]) => {
            const food = CompoFoods.find(f => f.id === comboId);
            return qty > 0 ? { content: food.content, price: food.price, quantity: qty } : null;
        }).filter(item => item !== null);

        onSelectCombos(updatedCombos); // Gọi hàm để cập nhật selectedCombos
    };

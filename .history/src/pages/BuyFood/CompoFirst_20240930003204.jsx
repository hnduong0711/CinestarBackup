...
const ListCombo = () => {
    ...
    const handleIncrease = (id) => {
        setQuantities((prev) => {
            ...
            if (newQuantity === 1) {
                const selectedFood = CompoFoods.find(food => food.id === id);
                onComboSelect(selectedFood); // Gọi hàm để truyền combo đã chọn
            }
    ...
    const handleDecrease = (id) => {
        setQuantities((prev) => {
            const newQuantity = Math.max((prev[id] || 0) - 1, 0); // Đảm bảo không giảm xuống dưới 0
            
            // Nếu số lượng mới là 0, loại bỏ combo khỏi danh sách đã chọn
            if (newQuantity === 0) {
                setSelectedCombos((prevCombos) => prevCombos.filter(combo => combo.id !== id));
            }
            
            return { ...prev, [id]: newQuantity }; // Cập nhật số lượng
        });
    };
    
    const handleQuantityChange = (id, value) => {
        const newValue = Math.max(Number(value) || 0, 0); // Đảm bảo giá trị tối thiểu là 0

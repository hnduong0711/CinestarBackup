                                ...
                                {groupcomboFoods[category].map((food) => {
                                    return (
                                        <div key={food.id} className="flex items-center mb-4 w-1/3">
                                            <div className="bg-[#ecf0f1] w-[184px] h-[231px] flex items-center justify-center relative overflow-hidden group"> {/* Thêm class group ở đây */}
                                                <div className="relative">
                                                    <img 
                                                        src={food.img} 
                                                        alt={food.title} 
                                                        className="w-full h-full object-cover transition-transform duration-300 group-hover:rotate-[5deg] group-hover:-translate-x-1" // Thêm hiệu ứng dịch chuyển
                                                    />
                                                </div>
                                            </div>
                                            <div className="flex-grow text-left ml-4">
                                                <h4 className="font-medium text-white text-[18px] transition-colors duration-300 group-hover:text-yellow-500">{food.title}</h4> {/* Thêm class group-hover ở đây */}
                                                <p className="text-white text-[16px] transition-colors duration-300 group-hover:text-yellow-500">{food.content}</p> {/* Thêm class group-hover ở đây */}
